...
- (void)viewDidLoad {
    ...
    if(![[self returnNickName] isEqualToString:@"0"])
    {
        textField.text = [self returnNickName];
    }
...
-(void)updateNickname
{
    NSDictionary *dic = @{@"appkey":APPkey,@"nickname":textField.text,@"mid":[self returnMid]};
...
#pragma dic MD5
    ...
    [manager POST:kUpdate parameters:Ndic progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        _loadingHud = nil;
        [self setNamewithNickName:textField.text];
        [self pop];
